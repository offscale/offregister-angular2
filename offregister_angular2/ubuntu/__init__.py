...
def install_nodejs0(*args, **kwargs):
    ...
    if not exists('$HOME/n'):
        ...
        run('curl -L https://git.io/n-install -o {remote_loc}'.format(remote_loc=remote_loc))
...
def _get_server_dir(**kwargs):
    return kwargs.get('SERVER_LOCATION', '/var/www/static/{}'.format(
        kwargs.get('nginx-init-context', {}).get(
            'NAME_OF_BLOCK', kwargs.get('nginx-conf-context', {}).get('NAME_OF_BLOCK', _get_repo_name(kwargs)))
    ))


def _validate(f):
    def validate(*args, **kwargs):
        required = (('GIT_REPO', 'https://github.com/org/git-repo.git'),
...
def git_static1(*args, **kwargs):
    ...
    clone_or_update(repo=kwargs['GIT_REPO'], to_dir=server_dir,
                    branch=kwargs.get('GIT_BRANCH', 'master'), use_sudo=True)
...
def _start_node2(*args, **kwargs):
    ...
    with cd(server_dir), shell_env(SOCK_URL=kwargs['SERVER_LOCATION']):
        ...
        s = sio.read()
        s.replace("app.set('SOCK_URL', undefined);",
                  "app.set('SOCK_URL', '{SERVER_LOCATION}');".format(SERVER_LOCATION=kwargs['SERVER_LOCATION']))
        sio = StringIO(s)
...
def setup_nginx_conf2(*args, **kwargs):
    ...
    _setup_nginx_conf2(
        ...
        **{'nginx-conf-file': path.join(get_python_lib(), 'offregister_nginx', 'conf', 'nginx.static.conf'),
           'nginx-conf-filename': kwargs['nginx-conf-filename'],
           'nginx-conf-context': {'SERVER_LOCATION': server_dir,
